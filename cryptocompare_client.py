...
class CryptoCompareClient:
    ...
    def get_top_coins(self, limit: int = 200) -> List[str]:
        ...
        top_100 = [
            'BTC', 'ETH', 'USDT', 'BNB', 'SOL', 'USDC', 'XRP', 'DOGE', 'HYPE', 'ADA',
            'TRX', 'AVAX', 'SHIB', 'TON', 'LINK', 'DOT', 'BCH', 'LTC', 'UNI', 'NEAR',
            'ICP', 'PEPE', 'HBAR', 'APT', 'FET', 'ETC', 'XLM', 'INJ', 'CRO', 'RNDR',
            'ATOM', 'ARB', 'IMX', 'MKR', 'FIL', 'VET', 'RUNE', 'XMR',
            'ALGO', 'AAVE', 'GRT', 'THETA', 'FTM', 'SAND', 'MANA', 'EOS', 'AXS',
            'EGLD', 'FLOKI', 'CHZ', 'NEO', 'MINA', 'KAVA', 'SNX', 'GALA', 'QNT', 'CFX',
            'ZEC', 'DASH', 'COMP', 'LDO', '1INCH', 'ENJ', 'CAKE', 'BAT',
            'ZIL', 'CRV', 'YFI', 'HNT', 'SUSHI', 'CVX', 'DYDX', 'KSM', 'MASK',
            'RAY', 'WIF', 'BONK', 'TIA', 'SEI', 'JUP', 'WLD', 'PYTH', 'STRK', 'BLUR',
            'ENS', 'LRC', 'OCEAN', 'MATIC', 'ROSE', 'AUDIO', 'ANKR', 'JASMY', 'IOTX', 'CELO'
        ]
        
        # Must-include coins that may not be in top 100 but we want
        must_include = ['LEO', 'SUI', 'MNT', 'POL', 'VVV', 'DEXE']
        
        # Combine and dedupe
        all_coins = top_100 + [c for c in must_include if c not in top_100]
        
        # Don't truncate must_include coins even if over limit
        # Must-include coins use CoinGecko fallback, so they're critical
        
        print(f"    Hardcoded list: {len(all_coins)} coins (top 100 + must-include)")
        return all_coins
    ...
    def get_weekly_data(self, symbol: str, weeks: int = 20) -> List[float]:
        ...
        if symbol in COINGECKO_FALLBACK:
            try:
                ...
                time.sleep(25)  # CoinGecko free tier: wait 25 seconds between calls to be safe
