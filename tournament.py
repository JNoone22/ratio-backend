...
def calculate_individual_ma_distance(prices: List[float]) -> Dict:
    ...
    percent_from_ema8 = 0
    if ema8:
        percent_from_ema8 = ((current_price - ema8) / ema8) * 100
        distance_from_ema8 = abs(percent_from_ema8) / 100
        near_ema8 = distance_from_ema8 <= 0.02  # Within 2%
    
    # Calculate trend weeks for BREAKOUT filter
    # Count consecutive weeks price is above MA, starting from most recent
    weeks_trending = 0
    if len(prices) >= config.MA_PERIOD + 1:  # Need at least 21 weeks to check week 0 vs week 1
        # For each week going backwards, check if above its MA
        for i in range(len(prices) - config.MA_PERIOD + 1):
            # Calculate MA for this week (includes this week and 19 before it)
            ma_window = prices[i : i + config.MA_PERIOD]
            week_ma = sum(ma_window) / config.MA_PERIOD
            
            # Check if this week is above MA
            if prices[i] > week_ma:
                weeks_trending += 1
            else:
    ...
    return {
        ...
        'percent_from_ema8': round(percent_from_ema8, 2),
