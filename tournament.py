...
def calculate_individual_ma_distance(prices: List[float]) -> Dict:
    ...
    # For each week, calculate the 20W SMA at that point and check if price > SMA
    weeks_trending = 0
    for i in range(len(prices)):
        # Need at least 20 weeks of history to calculate MA
        if i + config.MA_PERIOD > len(prices):
            break
        
        # Calculate 20W SMA for this week using the 20 weeks including and before it
        week_prices = prices[i:i + config.MA_PERIOD]
        week_ma = sum(week_prices) / config.MA_PERIOD
        
        # Check if this week's price closed above its MA
        if prices[i] > week_ma:
            weeks_trending += 1
        else:
