...
def calculate_individual_ma_distance(prices: List[float]) -> Dict:
    ...
    # Breakout = current week above MA AND last week below MA
    weeks_trending = 0
    
    if len(prices) >= 2:  # Need at least 2 weeks to compare
        current_week = prices[0]
        last_week = prices[1]
        
        # Check if we broke out this week (crossed above MA)
        if current_week > ma and last_week <= ma:
            weeks_trending = 1  # Fresh breakout
        elif current_week > ma:
            weeks_trending = 2  # Been above for a while (not a fresh breakout)
        else:
            weeks_trending = 0  # Below MA
