...
def calculate_ema(prices: List[float], period: int) -> float:
    ...
    prices array format: [newest, week1, week2, ..., oldest]
    """
    if len(prices) < period:
        return None
    ...
    # Start with SMA of the oldest 'period' weeks
    # prices[-period:] gives us the oldest 'period' prices
    oldest_prices = prices[-period:]
    ema = sum(oldest_prices) / period
    
    # Now iterate forward from oldest to newest
    # Process prices from index (len-period-1) down to 0 (newest)
    for i in range(len(prices) - period - 1, -1, -1):
...
def wins_matchup(asset_a_prices: List[float], asset_b_prices: List[float]) -> bool:
    ...
    # Use >= to handle exact ties (rare but possible)
...
def calculate_individual_ma_distance(prices: List[float]) -> Dict:
    ...
    Also calculates EMA8 and weeks trending (consecutive weeks above MA)
    Used as tiebreaker
    """
    ...
    # Count consecutive weeks above MA (from most recent backwards)
    # Each week's SMA is calculated from the 20 weeks BEFORE that week
    weeks_trending = 0
    
    # DEBUG
    print(f"  TREND DEBUG: len(prices)={len(prices)}, MA_PERIOD={config.MA_PERIOD}, loop range={len(prices) - config.MA_PERIOD}")
    
    for i in range(len(prices) - config.MA_PERIOD):
        # Calculate SMA of the 20 weeks BEFORE this week
        # Week i uses SMA of weeks [i+1 : i+21]
        ma_window = prices[i + 1 : i + 1 + config.MA_PERIOD]
        
        if len(ma_window) < config.MA_PERIOD:
            print(f"  TREND DEBUG: Breaking at i={i}, ma_window length={len(ma_window)}")
            break  # Not enough historical data
        
        week_ma = sum(ma_window) / config.MA_PERIOD
        
        # DEBUG first iteration
        if i == 0:
            print(f"  TREND DEBUG: price[0]={prices[i]:.2f}, MA={week_ma:.2f}, above={prices[i] > week_ma}")
        
        # Check if this week's price closed above its SMA
        if prices[i] > week_ma:
            weeks_trending += 1
        else:
            if i < 3:  # Log first few breaks
                print(f"  TREND DEBUG: Breaking at i={i}, price={prices[i]:.2f} <= MA={week_ma:.2f}")
            break  # Stop at first week below MA
    
    print(f"  TREND DEBUG: Final weeks_trending={weeks_trending}")
    # Check if price is near EMA8 (within 2%)
    near_ema8 = False
    if ema8:
        distance_from_ema8 = abs(current_price - ema8) / ema8
        near_ema8 = distance_from_ema8 <= 0.02  # Within 2%
    
    return {
        'current_price': round(current_price, 2),
        'ma': round(ma, 2),
        'ema8': round(ema8, 2) if ema8 else 0,
        'percent_above_ma': round(percent_above_ma, 2),
        'above_ma': current_price > ma,
        'above_ema8': current_price > ema8 if ema8 else False,
        'near_ema8': near_ema8,
        'weeks_trending': weeks_trending
    }
...
def calculate_tournament_rankings(assets_data: Dict[str, List[float]]) -> List[Dict]:
    ...
    for asset_a in symbols:
        for asset_b in symbols:
            ...
            # CRITICAL: Only evaluate each pair once!
