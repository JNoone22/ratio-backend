...
def calculate_individual_ma_distance(prices: List[float]) -> Dict:
    ...
    # Standard SMA calculation: each week's MA includes that week and the 19 before it
    weeks_trending = 0
    
    for i in range(len(prices) - config.MA_PERIOD + 1):
        # Calculate 20W SMA including this week
        # Week i uses MA of weeks [i : i+20]
        ...
        # Check if this week's price closed above its 20W SMA
        if prices[i] > week_ma:
            weeks_trending += 1
        else:
