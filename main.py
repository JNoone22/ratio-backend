...
def update_rankings(test_mode=False, asset_type=None, symbol=None, skip_crypto=False):
    ...
    try:
        ...
        if symbol:
            ...
            # For crypto updates: remove existing CRYPTO: entries to avoid duplicates
            if asset_type == 'crypto':
                existing_data = {k: v for k, v in existing_data.items() if not k.startswith('CRYPTO:')}
                print(f"  Cleared existing crypto from cache")
