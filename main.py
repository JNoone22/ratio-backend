...
def get_grid_matchups(grid_type):
    """
    Get head-to-head matchup results for a grid using synthetic pairs
    
    Args:
        grid_type: 'mag7', 'indices', or 'crypto'
    
    Returns:
        Grid matchup matrix with W/L results
    """
    # Define grid symbols
    grids = {
    ...
    all_assets_data = cache_manager.get_assets()
    
    # Filter to only symbols in this grid
    grid_data = {sym: all_assets_data.get(sym) for sym in symbols if sym in all_assets_data}
    
    if len(grid_data) < 2:
        return jsonify({'error': 'Insufficient data for grid'}), 503
    
    # Calculate matchups using synthetic pairs
    from tournament import wins_matchup
    
    matchups = {}
    for symbol_a in symbols:
        if symbol_a not in grid_data:
            continue
        matchups[symbol_a] = {}
        
        for symbol_b in symbols:
            if symbol_b not in grid_data or symbol_a == symbol_b:
                matchups[symbol_a][symbol_b] = None
                continue
            
            # Use synthetic pair comparison
            a_wins = wins_matchup(grid_data[symbol_a], grid_data[symbol_b])
            matchups[symbol_a][symbol_b] = 'W' if a_wins else 'L'
...
def get_grid(grid_type):
    """
    Calculate synthetic pair matchups for grid view
    Returns W/L results based on pair strength vs pair MA
    """
    # Define grid symbols
    grids = {
        'mag7': ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'TSLA', 'META'],
    ...
    # Calculate synthetic pair matchups
    matchups = {}
    ...
    for symbol1 in symbols:
        ...
        for symbol2 in symbols:
            ...
            # Calculate synthetic pair (ratio)
            min_length = min(len(prices1), len(prices2), config.MA_PERIOD)
            
            if min_length < config.MA_PERIOD:
                matchups[symbol1][symbol2] = 'N/A'
                continue
            
            # Calculate ratio for each week
            pair_ratios = []
            ...
            if len(pair_ratios) < config.MA_PERIOD:
                matchups[symbol1][symbol2] = 'N/A'
                continue
            
            # Calculate 20-week MA of the pair
            pair_ma = sum(pair_ratios[:config.MA_PERIOD]) / config.MA_PERIOD
            current_ratio = pair_ratios[0]  # Most recent
            
            # Determine winner: if current ratio > MA, symbol1 wins
            percent_diff = ((current_ratio - pair_ma) / pair_ma) * 100
            
            if current_ratio > pair_ma:
                matchups[symbol1][symbol2] = {
                    'result': 'W',
                    'percent': percent_diff
                }
            elif current_ratio < pair_ma:
                matchups[symbol1][symbol2] = {
                    'result': 'L', 
                    'percent': percent_diff
                }
