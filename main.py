...
def fetch_assets_by_type(asset_type: str, test_mode=False) -> dict:
    ...
    if asset_type == 'stocks':
        ...
        # Fetch top 200 crypto from CoinGecko
        crypto_limit = config.CRYPTO_LIMIT if hasattr(config, 'CRYPTO_LIMIT') else 200
        
        try:
            symbols = coingecko.get_top_coins(limit=crypto_limit)
            print(f"Fetching top {len(symbols)} cryptocurrencies from CoinGecko...")
        except Exception as e:
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                # Rate limiting - Wait 12 seconds between EVERY request = 5 calls/min
                if i < len(symbols) - 1:  # Don't wait after the last one
                    time.sleep(12)
...
def fetch_all_assets(test_mode=False, skip_crypto=False):
    ...
    # 3. Fetch crypto from CoinGecko (Top 200 by market cap)
    if skip_crypto:
        ...
        try:
            coingecko_symbols = coingecko.get_top_coins(limit=crypto_limit)
            print(f"  Got {len(coingecko_symbols)} crypto symbols from CoinGecko")
        except Exception as e:
            print(f"  ✗ ERROR getting symbols: {e}")
            coingecko_symbols = []
        
        if test_mode:
            coingecko_symbols = coingecko_symbols[:5]  # Just 5 in test mode
            print(f"⚡ TEST MODE: Limiting to {len(coingecko_symbols)} crypto")
        else:
            if len(coingecko_symbols) > 0:
                print(f"Loading top {len(coingecko_symbols)} cryptocurrencies by market cap...")
            else:
        ...
        for i, symbol in enumerate(coingecko_symbols):
            try:
                prices = coingecko.get_weekly_data(symbol, weeks=config.MA_PERIOD)
                if len(prices) >= config.MA_PERIOD:
                    all_data[symbol] = prices
                ...
                # Rate limiting - Wait 12 seconds between EVERY request = 5 calls/min
                if i < len(coingecko_symbols) - 1:  # Don't wait after the last one
                    time.sleep(12)
                
                # Progress update every 10
                if (i + 1) % 10 == 0:
                    print(f"  Processed {i + 1}/{len(coingecko_symbols)} crypto ({crypto_loaded} loaded, {crypto_failed} failed)...")
                
            except Exception as e:
                # Print first few errors to see what's happening
                if crypto_failed < 3:
                    print(f"  ✗ {symbol}: {str(e)}")
                errors.append(f"{symbol}: {str(e)}")
                crypto_failed += 1
        
        print(f"✓ Crypto loading complete: {crypto_loaded} loaded, {crypto_failed} failed out of {len(coingecko_symbols)} total")
