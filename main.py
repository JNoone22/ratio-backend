...
def fetch_assets_by_type(asset_type: str, test_mode=False) -> dict:
    ...
    if asset_type == 'stocks':
        ...
        for symbol in symbols:
            try:
                prices = binance.get_weekly_data(symbol, weeks=config.MA_PERIOD)
                if len(prices) >= config.MA_PERIOD:
                    all_data[symbol] = prices
            except:
                pass
...
def fetch_all_assets(test_mode=False):
    ...
    # 3. Fetch crypto from Binance (Top 200 by volume)
    print("\n🪙 Fetching cryptocurrency data from Binance...")
    
    # Get top cryptocurrencies by 24h trading volume
    crypto_limit = config.CRYPTO_LIMIT if hasattr(config, 'CRYPTO_LIMIT') else 200
    binance_symbols = binance.get_top_by_volume(limit=crypto_limit)
    
    if test_mode:
        binance_symbols = binance_symbols[:5]  # Just 5 in test mode
        print(f"⚡ TEST MODE: Limiting to {len(binance_symbols)} crypto")
    else:
        print(f"Loading top {len(binance_symbols)} cryptocurrencies by volume...")
        print(f"  Top 10: {', '.join(binance_symbols[:10])}")
    ...
    for i, symbol in enumerate(binance_symbols):
        try:
            ...
            # Rate limiting - Binance allows 1200 req/min = 20 req/sec
            if (i + 1) % 20 == 0:
                print(f"  Processed {i + 1}/{len(binance_symbols)} crypto ({crypto_loaded} loaded)...")
                time.sleep(1)  # Brief pause every 20 requests
            
        except Exception as e:
            # Don't print every error, just track it
            errors.append(f"{symbol}: {str(e)}")
    
    print(f"✓ Successfully loaded {crypto_loaded}/{len(binance_symbols)} cryptocurrencies")
...
def background_update_task(test_mode=False, asset_type=None, symbol=None):
    """
    ...
    try:
        ...
        update_rankings(test_mode=test_mode, asset_type=asset_type, symbol=symbol)
...
def trigger_update():
    ...
    Examples:
        /api/update - Full update (all assets, runs in background)
        /api/update?test=true - Fast test mode
        /api/update/status - Check progress
    """
    global update_status
    ...
    thread = threading.Thread(
        ...
        args=(test_mode, asset_type, symbol),
        daemon=True
    )
    thread.start()
