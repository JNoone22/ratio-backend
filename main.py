    ...
    if asset_type == 'stocks':
        ...
        for i, symbol in enumerate(symbols):
            try:
                ...
                if len(prices) >= config.MA_PERIOD:
                    # Prefix with CRYPTO: to avoid conflicts
                    crypto_key = f"CRYPTO:{symbol}"
                    all_data[crypto_key] = prices
                    crypto_loaded += 1
    ...
    if skip_crypto:
        ...
        for i, symbol in enumerate(cryptocompare_symbols):
            try:
                ...
                prices = cryptocompare.get_weekly_data(symbol, weeks=config.MA_PERIOD)
                if len(prices) >= config.MA_PERIOD:
                    # Prefix with CRYPTO: to avoid conflicts with stock symbols
                    crypto_key = f"CRYPTO:{symbol}"
                    all_data[crypto_key] = prices
                    crypto_loaded += 1
...
def update_rankings(test_mode=False, asset_type=None, symbol=None, skip_crypto=False):
    ...
    try:
        ...
        for asset in all_rankings:
            ...
            # Check if this is a crypto asset (prefixed with CRYPTO:)
            if symbol.startswith('CRYPTO:'):
                ...
                if is_forex:
                    ...
                    # Everything else is stock (shouldn't happen)
                    asset['type'] = 'stock'
    ...
    try:
        ...
        for symbol1 in symbols:
            ...
            for symbol2 in symbols:
                ...
                # For all other grids: look up prices directly
